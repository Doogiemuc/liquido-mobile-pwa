	...
	i18n: {
		messages: {
			...
			de: {
				...
				addProposalInfo:
					"Du kannst deinen eigenen Wahlvorschlag zu dieser Abstimmung hinzufügen.",
	...
	computed: {
		pageTitleLoc() {
			if (!this.poll.proposals || this.poll.proposals.length === 0)
				return this.$t("newPoll")
			if (this.poll.status === "ELABORATION")
				return this.$t("pollInElaboration")
		...
		showAddProposal() {
			...
			return (
				this.poll.proposals.filter(
					(prop) => prop.createdBy.email === currentUserEmail
				).length === 0
			)
		},
		showStartVotingPhase() {
			return (
				this.userIsAdmin &&
				this.poll.status === "ELABORATION" &&
				this.poll.proposals &&
				this.poll.proposals.length > 0
			)
		},
	},
	methods: {
